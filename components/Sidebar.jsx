  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? "p-4 w-64" : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
        <div>
            ...
            <button className={`mt-8 flex items-center justify-center cursor-pointet ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-3 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover-bg-gray-500/30 rounded-lg"}`}>
        ...
        <div onClick={ user ? null : openSignIn} className={`flex items-center ${expand ? 'hover:bg-white/10 rounded-lg' : 'justify-center w-full'} gap-3 text-white/60 text-sm mt-2 p-2 cursor-pointer`}>
        {
            user ? <UserButton/>
